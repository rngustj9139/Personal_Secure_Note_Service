package Koo.personalsecurenotewebservice.security.note;

import Koo.personalsecurenotewebservice.note.Note;
import Koo.personalsecurenotewebservice.note.NoteRepository;
...
class NoteControllerTest {
    ...
    private MockMvc mockMvc; // 서버를 진짜 띄우고 테스트를 하는것이 아닌 재현을 통한 테스트 수행
    private User user;
    private User admin;
    ...
    void deleteNote_인증없음() throws Exception {
        ...
        mockMvc.perform(delete("/note?id=" + note.getId()).with(csrf()))
                .andExpect(redirectedUrlPattern("**/login"))
                .andExpect(status().is3xxRedirection());
    }
